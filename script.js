...
function initMap() {
    // Coordinates for Gangrai, Chiplun, Ratnagiri, Maharashtra
    // Using approximate coordinates - you may need to adjust these
    const gangraiLocation = { lat: 17.5333, lng: 73.5094 };
    
    const mapElement = document.getElementById('map');
    if (!mapElement) return;
    
    // Create map centered on Gangrai
    const map = new google.maps.Map(mapElement, {
        zoom: 13,
        center: gangraiLocation,
        mapTypeId: 'roadmap',
        styles: [
            {
                featureType: 'poi',
    ...
    const marker = new google.maps.Marker({
        position: gangraiLocation,
        map: map,
        title: 'Gangrai Gram Panchayat',
    ...
    const infoWindow = new google.maps.InfoWindow({
        content: `
            <div style="padding: 10px;">
                <h3 style="margin: 0 0 10px 0; color: #1e40af; font-weight: 700;">ग्रामपंचायत गंगराई</h3>
                <p style="margin: 5px 0;"><strong>Gangrai Gram Panchayat</strong></p>
    ...
    if (typeof google !== 'undefined' && google.maps && google.maps.Geocoder) {
        ...
        geocoder.geocode(
            { address: 'Gangrai, Chiplun, Ratnagiri, Maharashtra, India' },
    ...
    window.gm_authFailure = function() {
        ...
        if (mapContainer && !mapContainer.querySelector('iframe')) {
            ...
            mapContainer.innerHTML = `
                <iframe 
                    src="https://www.google.com/maps?q=Gangrai,+Chiplun,+Ratnagiri,+Maharashtra,+India&output=embed" 
                    width="100%" 
                    height="100%" 
                    style="border:0;" 
                    allowfullscreen="" 
...
document.addEventListener('DOMContentLoaded', function() {
    const animatedElements = document.querySelectorAll('.overview-card, .highlight-card, .gallery-item, .population-table-container');
